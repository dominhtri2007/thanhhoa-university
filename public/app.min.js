...
chartRevenue = new Chart(ctx, {
  type: "line",
  data: { labels, datasets: [{ label: "Doanh thu theo ngày", data: values, borderColor: "#3a82f7", backgroundColor: "rgba(58,130,247,0.2)", fill: true, tension: 0.2 }] },
...
chartWeekly = new Chart(ctx, {
  type: "bar",
  data: { labels, datasets: [{ label: "Doanh thu 7 ngày gần nhất", data: values, backgroundColor: "rgba(58,130,247,0.6)" }] },
  options: { scales: { x: { ticks: { color: "#fff" }, grid: { color: "#333" } }, y: { ticks: { color: "#fff" }, grid: { color: "#333" } } } }
});
  } else {
...
chartHourly = new Chart(ctx, {
  type: "bar",
  data: { labels, datasets: [{ label: "Số tin nhắn theo giờ", data, backgroundColor: "rgba(255,193,7,0.6)" }] },
  options: { scales: { x: { ticks: { color: "#fff" }, grid: { color: "#333" } }, y: { ticks: { color: "#fff" }, grid: { color: "#333" } } } }
});
  } else {
...
socket.on("statisticResult", ({ count, total }) => {
  ...
  box.innerHTML = `
    <p><b>📌 Tổng hóa đơn:</b> ${count}</p>
    <p><b>💰 Tổng số tiền:</b> ${total.toLocaleString()} VNĐ</p>
    <p><b>📊 Doanh thu trung bình/ngày:</b> ${Math.round(total / 26).toLocaleString()} VNĐ</p>
  `;
});
